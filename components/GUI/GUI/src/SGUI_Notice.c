...
/** Notice:         None.                                               **/
/*************************************************************************/
SGUI_SIZE SGUI_Notice_Repaint(SGUI_SCR_DEV *pstDeviceIF,
                              SGUI_NOTICT_BOX *pstObject,
                              const SGUI_FONT_RES *pstFontRes,
                              SGUI_INT uiTextOffset)
{
    /*----------------------------------*/
    /* Variable Declaration             */
    /*----------------------------------*/
    SGUI_SIZE uiTextLines = 0;
    ...
    if ((NULL != pstObject) && (NULL != pstObject->cszNoticeText))
    {
        ...
        SGUI_Basic_DrawRectangle(
            pstDeviceIF, pstObject->stLayout.iX, pstObject->stLayout.iY,
            pstObject->stLayout.iWidth, pstObject->stLayout.iHeight,
        ...
        {
            stTextDisplayArea.iX =
                pstObject->stLayout.iX + pstObject->pstIcon->iWidth + 4;
            stTextDisplayArea.iWidth =
                pstObject->stLayout.iWidth - pstObject->pstIcon->iWidth - 6;
            ...
            stIconPosition.iY = 0;
            // Paint icon.
            SGUI_Basic_DrawBitMap(pstDeviceIF, &stIconDisplayArea,
                                  &stIconPosition, pstObject->pstIcon,
                                  SGUI_DRAW_NORMAL);
        }
        // Draw text;
        uiTextLines = SGUI_Text_DrawMultipleLinesText(
            pstDeviceIF, pstObject->cszNoticeText, pstFontRes,
            &stTextDisplayArea, uiTextOffset, SGUI_DRAW_NORMAL);
    }
    return uiTextLines;
}
...
void SGUI_Notice_FitArea(SGUI_SCR_DEV *pstDeviceIF, SGUI_RECT *pstFitArea)
{
    ...
    if ((NULL != pstDeviceIF) && (NULL != pstFitArea))
    {
        ...
        pstFitArea->iWidth =
            pstDeviceIF->stSize.iWidth - (SGUI_NOTICE_MARGIN << 1);
        pstFitArea->iHeight =
            pstDeviceIF->stSize.iHeight - (SGUI_NOTICE_MARGIN << 1);
    }
}
#endif