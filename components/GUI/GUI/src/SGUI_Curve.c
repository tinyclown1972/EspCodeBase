...
/** Notice:         none.                                               **/
/*************************************************************************/
void SGUI_Curve_Initialize(SGUI_CURVE_STRUCT *pstObj,
                           const SGUI_CURVE_PARAM *pcstInitParam)
{
    /*----------------------------------*/
    /* Process                          */
    /*----------------------------------*/
    ...
    {
        SGUI_SystemIF_MemoryCopy(&(pstObj->stParam), (void *)pcstInitParam,
                                 sizeof(SGUI_CURVE_PARAM));
        SGUI_SystemIF_MemorySet(&(pstObj->stData), 0x00,
                                sizeof(SGUI_CURVE_DATA));
    }
}
...
/** Return:         None.                                               **/
/*************************************************************************/
void SGUI_Curve_Repaint(SGUI_SCR_DEV *pstDeviceIF,
                        const SGUI_CURVE_STRUCT *pstObj)
{
    /*----------------------------------*/
    ...
    {
        /* Clean up background. */
        SGUI_Basic_DrawRectangle(pstDeviceIF, pstObj->stParam.stLayout.iX,
                                 pstObj->stParam.stLayout.iY,
                                 pstObj->stParam.stLayout.iWidth,
                                 pstObj->stParam.stLayout.iHeight,
                                 SGUI_COLOR_BKGCLR, SGUI_COLOR_BKGCLR);
        if (pstObj->stData.stPoints.sCount > 1)
        {
            ...
            {
                /* Paint visible line. */
                if (SGUI_Basic_PointIsInArea(
                        &(pstObj->stParam.stLayout),
                        pcstLineStartPoint->stPosition.iX,
                        pcstLineStartPoint->stPosition.iY) ||
                    SGUI_Basic_PointIsInArea(&(pstObj->stParam.stLayout),
                                             pcstLineStartPoint->stPosition.iX,
                                             pcstLineStartPoint->stPosition.iY))
                {
                    SGUI_Basic_DrawLine(
                        pstDeviceIF, pcstLineStartPoint->stPosition.iX,
                        pcstLineStartPoint->stPosition.iY,
                        pcstLineEndPoint->stPosition.iX,
                        pcstLineEndPoint->stPosition.iY, SGUI_COLOR_FRGCLR);
                }
            ...
            while (NULL != pcstLineEndPoint)
            {
                ...
                if (SGUI_Basic_PointIsInArea(
                        &(pstObj->stParam.stLayout),
                        &(pcstLineStartPoint->stPosition)) ||
                    SGUI_Basic_PointIsInArea(&(pstObj->stParam.stLayout),
                                             &(pcstLineEndPoint->stPosition)))
                {
                    SGUI_Basic_DrawLine(
                        pstDeviceIF, pcstLineStartPoint->stPosition.iX,
                        pcstLineStartPoint->stPosition.iY,
...
/** Return:         None.                                               **/
/*************************************************************************/
void SGUI_Curve_InitializePoint(SGUI_CURVE_POINT *pstNewPoint, SGUI_INT iX,
                                SGUI_INT iY)
{
    /*----------------------------------*/
    /* Process                          */
...
/** Return:         None.                                               **/
/*************************************************************************/
void SGUI_Curve_AddPoint(SGUI_CURVE_STRUCT *pstObj,
...
/*************************************************************************/
SGUI_BOOL SGUI_Curve_InsertBeforePoint(SGUI_CURVE_STRUCT *pstObj,
                                       SGUI_CURVE_POINT *pstNewPoint)
{
    /*----------------------------------*/
    /* Variable Declaration             */
...
/*************************************************************************/
SGUI_BOOL SGUI_Curve_InsertAfterPoint(SGUI_CURVE_STRUCT *pstObj,
                                      SGUI_CURVE_POINT *pstNewPoint)
{
    /*----------------------------------*/
    /* Variable Declaration             */
...
/*************************************************************************/
SGUI_BOOL SGUI_Curve_RemovePoint(SGUI_CURVE_STRUCT *pstObj,
                                 SGUI_CURVE_POINT *pstPoint)
{
    /*----------------------------------*/
    /* Variable Declaration             */
    /*----------------------------------*/
    ...
    if ((NULL != pstObj) && (NULL != pstPoint) &&
        (pstObj->stData.stPoints.sCount > 2))
    {
...
/** Return:         None.                                               **/
/*************************************************************************/
void SGUI_Curve_CalculatePointPosition(SGUI_CURVE_STRUCT *pstObj,
                                       SGUI_CURVE_POINT *pstPoint)
{
    /*----------------------------------*/
    /* Process                          */
    ...
    if ((NULL != pstObj) && (NULL != pstPoint))
    {
        pstPoint->stPosition.iX =
            (SGUI_RECT_WIDTH(pstObj->stParam.stLayout) - 1) *
            (pstPoint->stPoint.iX - pstObj->stParam.stXRange.iMin) /
            (SGUI_RANGE_SIZE(pstObj->stParam.stXRange) - 1);
        pstPoint->stPosition.iY =
            (SGUI_RECT_HEIGHT(pstObj->stParam.stLayout) - 1) *
            (pstPoint->stPoint.iY - pstObj->stParam.stYRange.iMin) /
            (SGUI_RANGE_SIZE(pstObj->stParam.stYRange) - 1);
...
/*************************************************************************/
SGUI_CURVE_POINT *SGUI_Curve_FocusPoint(SGUI_CURVE_STRUCT *pstObj,
                                        SGUI_INT iIndex)
{
    /*----------------------------------*/
    /* Variable Declaration             */
...
/*************************************************************************/
SGUI_BOOL SGUI_Curve_UpdateFocusPoint(SGUI_CURVE_STRUCT *pstObj, SGUI_INT iX,
                                      SGUI_INT iY)
{
    /*----------------------------------*/
    /* Variable Declaration             */
    ...
    else
    {
        do
        {
            ...
            /* Check point X value. */
            if ((pstObj->stData.pstFocused ==
                 pstObj->stData.stPoints.pstHead) ||
                (pstObj->stData.pstFocused == pstObj->stData.stPoints.pstEnd))
            {
...
/** Return:         None.                                               **/
/*************************************************************************/
void SGUI_Curve_HighlightFocus(SGUI_SCR_DEV *pstDeviceIF,
                               SGUI_CURVE_STRUCT *pstObj)
{
    /*----------------------------------*/
    /* Variable Declaration             */
    /*----------------------------------*/
    ...
    if ((NULL != pstDeviceIF) && (NULL != pstObj))
    {
        if (NULL != pstObj->stData.pstFocused)
        {
            ...
            if (SGUI_RECT_X_START(stFocusMarkArea) <=
                SGUI_RECT_X_START(pstObj->stParam.stLayout))
            {
                stFocusMarkArea.iWidth -=
                    (SGUI_RECT_X_START(pstObj->stParam.stLayout) -
                     stFocusMarkArea.iX);
                stFocusMarkArea.iX =
                    SGUI_RECT_X_START(pstObj->stParam.stLayout);
            ...
            if (SGUI_RECT_X_END(stFocusMarkArea) >=
                SGUI_RECT_X_END(pstObj->stParam.stLayout))
            {
                stFocusMarkArea.iWidth -=
                    (SGUI_RECT_X_END(stFocusMarkArea) -
                     SGUI_RECT_X_END(pstObj->stParam.stLayout));
            }

            if (SGUI_RECT_Y_START(stFocusMarkArea) <=
                SGUI_RECT_Y_START(pstObj->stParam.stLayout))
            {
                stFocusMarkArea.iHeight -=
                    (SGUI_RECT_Y_START(pstObj->stParam.stLayout) -
                     stFocusMarkArea.iY);
                stFocusMarkArea.iY =
                    SGUI_RECT_Y_START(pstObj->stParam.stLayout);
            ...
            if (SGUI_RECT_Y_END(stFocusMarkArea) >=
                SGUI_RECT_Y_END(pstObj->stParam.stLayout))
            {
                stFocusMarkArea.iHeight -=
                    (SGUI_RECT_Y_END(stFocusMarkArea) -
                     SGUI_RECT_Y_END(pstObj->stParam.stLayout));
            }
            /* Paint focused mark. */
            SGUI_Basic_DrawRectangle(pstDeviceIF, stFocusMarkArea.iX,
                                     stFocusMarkArea.iY, stFocusMarkArea.iWidth,
                                     stFocusMarkArea.iHeight, SGUI_COLOR_FRGCLR,
                                     SGUI_COLOR_FRGCLR);
        }
    }
}
...
/*************************************************************************/
SGUI_INT SGUI_Curve_GetDependentValue(SGUI_CURVE_STRUCT *pstObj,
                                      SGUI_INT iArgument)
{
    /*----------------------------------*/
    /* Variable Declaration             */
    ...
    if (pstObj)
    {
        ...
        while ((NULL != pstAreaStart) && (NULL != pstAreaEnd))
        {
            ...
            {
                iDependent =
                    (iArgument - pstAreaStart->stPoint.iX) *
                        (pstAreaEnd->stPoint.iY - pstAreaStart->stPoint.iY) /
                        (pstAreaEnd->stPoint.iX - pstAreaStart->stPoint.iX) +
                    pstAreaStart->stPoint.iY;
                break;
            }
        }
    }
