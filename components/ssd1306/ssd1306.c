...
static SSD1306_t* displayDev = NULL;

void DisplayInit(SSD1306_t *dev)
{
    bool retVal = false;
    ...
    ssd1306_init(dev, SSD_1306_WIDTH, SSD_1306_HEIGHT);
}
...
void ssd1306_show_buffer(SSD1306_t* dev)
{
    if (dev->_address == SPI_ADDRESS)
    {
        ...
        {
            spi_display_image(dev, page, 0, dev->_page[page]._segs,
                              dev->_width);
        }
    }
    else
    {
        for (int page = 0; page < dev->_pages; page++)
        {
            i2c_display_image(dev, page, 0, dev->_page[page]._segs,
                              dev->_width);
        }
    }
}
    ...
    else if (scroll == SCROLL_UP)
    {
        ...
        for (int page = 0; page < dev->_pages - 1; page++)
        {
            ...
            for (int seg = _start; seg <= _end; seg++)
            {
                ...
                if (seg == 0)
                {
                    ESP_LOGD(TAG, "b page=%d wk0=%02x wk1=%02x", page, wk0,
                             wk1);
                }
                wk0 = wk0 >> 1;
                wk1 = wk1 & 0x01;
                ...
                if (seg == 0)
                {
                    ESP_LOGD(TAG, "a page=%d wk0=%02x wk1=%02x wk2=%02x", page,
                             wk0, wk1, wk2);
                }
                if (dev->_flip)
    ...
    for (int _height = 0; _height < height; _height++)
    {
        for (int index = 0; index < _width; index++)
        {
            for (int srcBits = 7; srcBits >= 0; srcBits--)
            {
                ...
                // wk2 = ssd1306_copy_bit(bitmap[index+offset], srcBits, wk0,
                // dstBits);
                wk2 = ssd1306_copy_bit(wk1, srcBits, wk0, dstBits);
                if (dev->_flip)
                    wk2 = ssd1306_rotate_byte(wk2);

                ESP_LOGD(TAG, "index=%d offset=%d page=%d _seg=%d, wk2=%02x",
...
void ssd1306_clear_screen_x()
{
    ...
    char space[16];
    memset(space, 0x00, sizeof(space));
    for (int page = 0; page < displayDev->_pages; page++)
    {
        ssd1306_display_text(displayDev, page, space, sizeof(space), false);
    }
}

// Set pixel to internal buffer. Not show it.
void ssd1306_set_pixel_x(int xPos, int yPos, int iColor)
{
    if(displayDev == NULL)
    {
        printf("Display not initialized\n");
    ...
    uint8_t _seg = xPos;
    uint8_t wk0 = displayDev->_page[_page]._segs[_seg];
    ...
    displayDev->_page[_page]._segs[_seg] = wk0;
}

// Get pixel in internal buffer.
int ssd1306_get_pixel_x(int xPos, int yPos)
{
    if(displayDev == NULL)
    {
        printf("Display not initialized\n");
...
void _ssd1306_pixel(SSD1306_t* dev, int xpos, int ypos, bool invert)
{
    ...
    uint8_t wk1 = 1 << _bits;
    ESP_LOGD(TAG, "ypos=%d _page=%d _bits=%d wk0=0x%02x wk1=0x%02x", ypos,
             _page, _bits, wk0, wk1);
    if (invert)
    {
        wk0 = wk0 & ~wk1;
